...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // ══════════════════════════════════════════════════════════════════
    const METRICOOL_NETWORKS = ['instagram', 'facebook', 'twitter', 'tiktok', 'linkedin', 'youtube', 'pinterest', 'google'];
    ...
    // Step B: Group resolved targets by dateTime → merge providers into single payload
    const groupedByTime = new Map<string, ResolvedTarget[]>();
    for (const rt of resolvedTargets) {
      const key = rt.dateTime;
      if (!groupedByTime.has(key)) groupedByTime.set(key, []);
      groupedByTime.get(key)!.push(rt);
    }

    diagnostics.push(`Grouped into ${groupedByTime.size} unified time slots (from ${resolvedTargets.length} targets)`);

    // Step C: Send one API call per grouped time slot
    for (const [dateTime, group] of groupedByTime.entries()) {
      try {
        // Use the first target's caption and media as the primary
        const primaryCaption = group[0].caption || '';
        const primaryMedia = group[0].mediaUrl || '';
        const timezone = group[0].timezone;

        // Build merged providers array
        ...
        for (const rt of group) {
          ...
          if (rt.target.platformData) {
            for (const [key, val] of Object.entries(rt.target.platformData)) {
              if (key.endsWith('Data')) {
                providerObj[key.replace('Data', '')] = val;
              }
            }
          }
          ...
          if (rt.target.metricoolNetwork === 'youtube') {
            if (!providerObj.youtube) providerObj.youtube = {};
            const ytData = providerObj.youtube as Record<string, unknown>;
          ...
          if (rt.target.metricoolNetwork === 'pinterest') {
            if (!providerObj.pinterest) providerObj.pinterest = {};
            const pinData = providerObj.pinterest as Record<string, unknown>;
          ...
          if (rt.target.metricoolNetwork === 'google') {
            if (!providerObj.google) providerObj.google = {};
            const gmbData = providerObj.google as Record<string, unknown>;
            gmbData.topicType = 'STANDARD';
          }

          providers.push(providerObj);
        }

        // Build unified payload with ALL providers for this time slot
        const scheduledPost: Record<string, unknown> = {
          ...
          shortener: false,
          draft: false,
          media: primaryMedia ? [primaryMedia] : [],
        };

        const platformNames = group.map(g => g.target.platform).join(', ');
        console.log(`[Smart Publish V7] ${platformNames} → POST (unified at ${dateTime})`);
        ...
        if (response.ok) {
          ...
          for (const rt of group) {
            results.push({
              ...
              success: true,
              postId: apiResult.id || apiResult.postId || 'scheduled',
              scheduledTime: dateTime,
            });
          }
          diagnostics.push(`[${platformNames}]: unified publish at ${dateTime}`);
        } else {
          for (const rt of group) {
            results.push({
              platform: rt.target.platform,
          ...
          diagnostics.push(`[${platformNames}]: FAILED — ${response.status}`);
        }
        ...
        for (const rt of group) {
          results.push({
            ...
            type: rt.target.mediaType,
            success: false,
            error: groupError instanceof Error ? groupError.message : 'خطأ غير معروف',
          });
        }
      }
    }
