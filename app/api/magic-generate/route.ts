...
import { generateEliteDirectorContent } from '@/lib/scenarioEngine';
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // ── Step 3: Generate Elite Director content ───────────────────────────
    diagnostics.push('Step 3: Generating elite creative content...');
    const directorOutput = await generateEliteDirectorContent(perfumeData, intel);
    diagnostics.push(`Director: ${directorOutput.cinematic_image_prompts.length} prompts, ${directorOutput.scenarios.length} scenarios`);

    // ── Step 4: Determine vibe/attire from scraped data ───────────────────
    const vibeMap: Record<string, string> = {
      floral: 'rose_garden',
      oriental: 'oriental_palace',
      fresh: 'ocean_breeze',
      woody: 'classic_library',
      sweet: 'royal_luxury',
    };
